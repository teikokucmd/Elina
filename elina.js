    ...
    if (!conn.authState.creds.registered) {  
      ...
      if (!!phoneNumber) {
        ...
        if (!Object.keys(PHONENUMBER_MCC || {}).some(v => numeroTelefono.startsWith(v))) {
          console.log(chalk.bgBlack(chalk.bold.greenBright(`Ingresa el número a vincular.\n${chalk.bold.yellowBright(`⏳  Ejemplo: 521657×××××××`)}\n${chalk.bold.magentaBright('---> ')}`)))
          process.exit(0)
        }
      } else {
        while (true) {
          numeroTelefono = await question(chalk.bgBlack(chalk.bold.greenBright(`Ingresa el número a vincular.\n⏳  Ejemplo: 521657×××××××\n`)))
      ...
      setTimeout(async () => {
        ...
        console.log(chalk.bold.white(chalk.bgMagenta(`Code Elina💮: `)), chalk.bold.white(chalk.white(codigo)))
      }, 3000)
    }
  }
}
...
async function connectionUpdate(update) {
  ...
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      console.log(chalk.bold.cyanBright(`\n⚠️ SIN CONEXIÓN, BORRE LA CARPETA ${sessions} Y ESCANEA EL CÓDIGO QR ⚠️`))
