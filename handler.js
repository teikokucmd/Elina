...
if (db.data.chats[m.chat].reaction && m.text.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify|bot|elina|a|s)/gi)) {
...
function pickRandom(list) { return list[Math.floor(Math.random() * list.length)]}
    ...
    const isAdmin = (await conn.groupMetadata(m.chat)).participants.filter(v => v.admin !== null).map(v => v.id).includes(m.sender)
